window.objects = (() => {
	...
	function addStaticObjects() {
		list.push(new House(new Vector(670, 0), undefined, false));
		list.push(new Owl(new Vector(670, 75)));
		...
		[100, 140, 200, 580, 850, 940, 8100, 8180, 8450, 8600, 24050, 24200, 24480, 24640].forEach((item) => {
			list.push(new Tree(new Vector(item, mountain.getHeight(item) - 5), 1));
		});
	}
			...
			list
				.filter((item) => {
					if (!gc.paused) {
						...
						return item.position.x >= camera.getView().start.x && item.position.x <= camera.getView().end.x &&
							(item instanceof Campfire || item instanceof Bench || item instanceof House);
					}
				})
				.forEach((item) => {
				item.r();
